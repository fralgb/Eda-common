package com.omniselling.security.authentication.preauth;
...
public class OmniPreAuthenticatedAuthenticationProvider implements AuthenticationProvider, InitializingBean, Ordered
{
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException
    {
        ...
        if (ud instanceof OmniSecUser)
        {
            OmniSecUser user = (OmniSecUser) ud;
            AttributePrincipal principal = (AttributePrincipal) authentication.getPrincipal();
            if (principal.getAttributes() != null)
            {
                user.setLocale((String) principal.getAttributes().get("locale"));
            }
        }
    ...
    /**
     * If true, causes the provider to throw a BadCredentialsException if the presented authentication request is
     * invalid (contains a null principal or credentials). Otherwise it will just return null. Defaults to false.
     */
    ...
    /**
     * Sets the strategy which will be used to validate the loaded <tt>UserDetails</tt> object for the user. Defaults to
     * an {@link AccountStatusUserDetailsChecker}.
     * 
